...
/// Preset values plus a "Custom" button that asks for a number within `allowed`.
struct PresetPicker: View {
  let title: String
  let presets: [Int]
  let prompt: String
  /// Accepted custom range. Without an upper bound a large value overflows the
  /// scoring arithmetic and traps. Mirrors the prompts in public/index.html.
  var allowed: ClosedRange<Int> = 1...Game.maxPoints
  var hint: String?
  ...
  var body: some View {
    ...
    .alert(prompt, isPresented: $isPrompting) {
      ...
      Button("OK") {
        if let n = Int(customText.trimmingCharacters(in: .whitespaces)), allowed.contains(n) { value = n }
      }
    }
  }
}
